/**
 * Receipt document builder — shared by the customer flow (index.html) and the
...
export function buildReceiptHtml(r, { duplicate = false } = {}) {
  ...
  const html = `
<!DOCTYPE html>
<html><head><meta charset="UTF-8"><title>Receipt ${r.order_id.slice(0, 8).toUpperCase()}</title>
<style>
...
export function printReceipt(r, opts = {}) {
  if (!r) { alert("No receipt to print."); return false; }
